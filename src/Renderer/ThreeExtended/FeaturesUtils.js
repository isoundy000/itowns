...
function filterFeatureUnderCoordinate(coordinate, type, coordinates, epsilon, properties, result) {
    if (type == 'linestring' && pointIsOverLine(coordinate, coordinates, epsilon)) {
        result.push({ type, properties, coordinates });
        ...
        if (closestPoint) {
            result.push({ type, properties, coordinates: closestPoint });
        }
    }
}
export default {
        ...
        if (features.geometries) {
            ...
            for (const feature of features.geometries) {
                ...
                for (const id in feature.featureVertices) {
                    ...
                    filterFeatureUnderCoordinate(coordinate, feature.type, coordinates, epsilon, properties, result);
                }
            }
            ...
            filterFeatureUnderCoordinate(coordinate, features.geometry.type, features.geometry.coordinates, epsilon, features.properties, result);
        }
        return result;
    },
};
